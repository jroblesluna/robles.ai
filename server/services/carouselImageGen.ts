...
export function buildArticleImagePrompt(articleTitle: string, categories: string[], palette?: PaletteConfig, imageStyle?: ImageStyleConfig, contentSummary?: string): string {
  ...
  return (
    ...
    `PROMINENTLY feature visual references to the companies, technologies, and figures mentioned in the article. These should be CLEARLY VISIBLE and central to the composition, not subtle background elements. For example: a car clearly resembling the mentioned EV brand's design language in the foreground, industrial equipment prominently displaying the mentioned company's brand colors or recognizable logo shapes, large server racks with recognizable cloud company aesthetics front and center, smartphones or devices clearly evoking mentioned tech brands. If monetary figures are mentioned, show them prominently through large growth charts, coin stacks, or dollar signs with scale indicators. Company and technology representations should occupy at least 40-50% of the visual space — they are the STARS of the image, not decoration. The references don't need to be pixel-perfect copies — but they should be immediately recognizable and eye-catching to someone familiar with the companies/technologies discussed. ` +
