...
- **⚠️ Cloud Run pendiente para LangChain**: al 2026-09, `langchain-api.robles.ai` **no resuelve (NXDOMAIN)** — a diferencia de `rag-api`/`identity-api`, el servicio de LangChain **no está desplegado ni mapeado a dominio** en Cloud Run. El frontend está listo y apunta a esa URL, pero la demo fallará (warm-up marca "cold", las llamadas dan error de red) hasta que se despliegue el API server de LangChain y se cree el domain mapping + DNS (CNAME a `ghs.googlehosted.com`), con `GET /` respondiendo y CORS permitiendo POST desde `https://robles.ai`. Ese repo del API server **no vive en este proyecto**.
